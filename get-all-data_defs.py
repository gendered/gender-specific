...
def addTermsToDictionary():
	def add(terms):
		words = {}
		for term in terms:
			definition = wordApi.getDefinitions(term, partOfSpeech='noun', limit=1)
			words[term] = definition[0].text
		return words

	femaleDict = add(dict.fromkeys(femaleTerms))
	maleDict = add(dict.fromkeys(maleTerms))
...
#
addTermsToDictionary()
getWebster()
getWordnik()
getDatamuse()

#
writeToJson('words/female-all-defs', femaleAll)
writeToJson('words/male-all-defs', maleAll)
